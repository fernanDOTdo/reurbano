...
/**
 * Reurbano/AnalyticsBundle/Document/TrackingRepository.php
 *
...
class TrackingRepository extends BaseRepository
{
    ...
    /**
     * Localiza um Tracking quando a Deal está apenas sendo visualizada
     *
     * @param Deal $deal, deal
     * @param User $city, cidade
     * @param Category $category, categoria
     ...
     * @return bool
     */
    public function findByTracking($deal, $city, $category, $inCookie = false){
    	return $this->createQueryBuilder()
    		->field('category.$id')->equals(new \MongoId($category->getId()))
	    	->field('city.$id')->equals(new \MongoId($city->getId()))
	    	->field('deal.$id')->equals(new \MongoId($deal->getId()))
    ...
    /**
     * Localiza um Tracking por parceiro
     *
     * @param Associado $associate, associado
     * @param Deal $deal, deal
     * @param User $city, cidade
     * @param Category $category, categoria
     * @param Boolean $inCookie, contém ou não em cookie
     ...
     * @return bool
     */
    public function findByTrackingByAssociate($associate, $deal, $city, $category, $inCookie = false){
    	return $this->createQueryBuilder()
    	->field('associate._id')->equals(new \MongoId($associate->getId()))
    	->field('category.$id')->equals(new \MongoId($category->getId()))
    	->field('city.$id')->equals(new \MongoId($city->getId()))
